...
def baseInfiltrated(env):

    finishLinePos = [sh//2 + finishLine.num_rows_or_height//2, 1 ]

    for zombie in env.zombie_list:
        zombiePos = [zombie.row, zombie.col]
        if(zombiePos == finishLinePos):
            return True
...
def main():
    ...
    while not baseInfiltrated(env): #TODO stop shit from going off screen and breaking the program lol
        key = window.getch() 
        if key == ord('p'): 
            curses.endwin()
            subprocess.call(["reset"])
